...
@celery.task(bind=True)
def process_hair_style(self, task_id: int):
    """处理发型编辑任务"""
    db = SessionLocal()
    ...
    try:
        ...
        input_data = json.loads(task.input_data)
        image_path = input_data["image_path"]
        hair_style = input_data["hair_style"]
        add_watermark = input_data["add_watermark"]
        
        logger.info(f"开始处理任务 {task_id}: 图片路径={image_path}, 发型={hair_style}, 水印={add_watermark}")
        ...
        try:
            ...
            form = {
                "req_key": "hair_style_generation",  # 假设这是火山引擎的发型编辑API接口
